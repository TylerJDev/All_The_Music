...
if (typeArtistAlbum === 'track') {
    ...
    // If the track isn't found, redirect to the 404 page
    if (tResponse.error === 6 && tResponse.message === 'Track not found') {
...
$.getJSON('https://ws.audioscrobbler.com/2.0/?method=artist.getinfo&artist=' + artistOf + '&lang=' + supportedLanguages[languageIndex] + '&api_key=' + lastAPIKey + '&format=json', function(response) {
	...
	if (bioBodyFPre === '') {
		wikiSummary = false;
		wikiLink = lastFM.artist.url + '/+wiki/edit'
	}

	var bioBodyF = bioBodyFPre.substring(0, bioBodyFPre.indexOf('<a href="'));

	bioFullLink = bioBodyFPre.substring(bioBodyFPre.indexOf('https://'), bioBodyFPre.indexOf('">'));
	...
	$.getJSON('https://ws.audioscrobbler.com/2.0/?method=artist.gettopalbums&artist=' + artistOf + '&limit=290&api_key=' + lastAPIKey + '&format=json', function(response) {
		...
		if (topAlbumsOf.topalbums.album.length >= 10) {
			var albumCount = 10; // Made in order to 'skip' rather than just a numerical value (i.e, 10)
		} else {
			var albumCount = topAlbumsOf.topalbums.album.length;
			albumArray = albumArray.splice(0, topAlbumsOf.topalbums.album.length);
		}
    ...
    // If there aren't any 'Similar Artists' don't render, else render
    var checkObj = 0;
    for (var checkIfObjEmpty of Object.values(similarArray)) {
    ...
    if (checkObj !== Object.keys(similarArray).length) {
      function SimilarCon() {
        return (
...
var AlbumGet = function(track) {
		...
		if (trackDetails === undefined) {
			...
			const TrackNotFound = () => (
			  <div id="four-oh-four" className="text-center">
				...
				<h2 id="404-header">TRACK NOT FOUND</h2>
			  </div>
			)

			ReactDOM.render(<TrackNotFound />, document.getElementById('main-body-container'));
		} else {
	...
	$.getJSON(getAlbumJson, function(response) {
		...
		wikiLink = albumDetails.albumWikiLink;
				...
				function SongDetails() {
					return (
						<div className="song-details-div">
							...
							<a href={trackDetails.trackLastFmLinks[0] + '/+wiki'} target="_blank" style={{marginLeft: '20px'}}>View on Last.FM</a>
							<p>{trackDetails.trackWiki.substring(0, trackDetails.trackWiki.indexOf('<a href="'))}</p>
						</div>
					);
				}
...
var reactRun = function() {
	...
	function ReadMoreBtn() {
		if (wikiSummary === true) {
			...
			return <a href={wikiLink} rel="noopener noreferrer" target="_blank"><button id="main-button" className="btn-type-1">Add Description</button></a>; // Return nothing!
		}
	}
	...
	function ReadMoreLink() {
		if (wikiSummary === true) {
			...
			return <a href={wikiLink} rel="noopener noreferrer" target="_blank">Add Description</a>;
		}
	}
			...
			<div>
				<div id="sidebar-details">
					<div>
						...
						<div id="sidebar-con-2">
							<h4>Social Media</h4>
							<hr></hr>
							<div id="social-media-sidebar">
								<p>No social media found for this {typeOf}</p>
							</div>
						</div>
					</div>
				</div>
	...
	if (typeOf === 'artist') {
		var tagDetails = headerDetails.tagsOf.tag;
	} else if (typeOf === 'album') {
		var tagDetails = albumDetails.albumTags.tag;
	}

	if (tagDetails !== undefined) {
		const artistGenres = tagDetails.map((tagDetails) =>
			<a href={currentLink + 'Charts?tag=' + tagDetails.name}className="tag-box">{tagDetails.name}</a>
		);

		ReactDOM.render(
			<div id="tag-con">{artistGenres}</div>,
			document.getElementsByClassName('artist-genres')[0]
		);
	} else {
		function ArtistGenres() {
			return <p>No genres available</p>
		}
		ReactDOM.render(<ArtistGenres />, document.getElementsByClassName('artist-genres')[0]);
	}
	...
	function OptTabs() {
		if (typeArtistAlbum === 'artist') { /* If type is 'artist', else it won't render */
			return <li className="nav-item"><a className="nav-link" data-toggle="tab" href="#albums-songs-tab" role="tab" aria-controls="albums-songs-tab">Albums</a></li>;
		}
	}
	...
	if (eventCount > 0) { // If there is more than '0' events
		const eventItems = eventsArray.map((eventsArray) =>
			<div className="col-xs-12 col-sm-12 col-md-6 event-box">
		...
		function BiographySection() {
			return (
				<div id="biography-section">
					<h1 className="main-body-header">Biography</h1>
					<a href={bioFullLink} target="_blank">Read on Last.FM</a>
	...
	function AlbumsSongs() {
			return (
				<div id="album-songs-section">
					<h2 className="main-body-header">Albums</h2>
	...
	//}
		...
		<div className="album-con">
			...
			<a href={currentLink + 'index.html?query=' + artistOf + '&album=' + pageHold[1] + '&type=album'} className="album-link">
				<div className="col-xs-12 col-sm-12 col-md-3 album-photo-col album-header">
	...
	$.getJSON('https://musicbrainz.org/ws/2/artist/' + mbidOf + '?inc=url-rels+aliases+tags+ratings+artist-rels&fmt=json', function(response) {
		...
		*/
		...
		for (var getRel = 0; getRel < musicBrainz.relations.length; getRel++) {
			...
			if (linkType === 'discogs' || linkType === 'youtube' || linkType === 'soundcloud' || linkType === 'streaming music' || linkType === 'bandcamp' || linkType === 'purchase for download') {
					...
					if (linkType === 'youtube') {
						...
						$.getJSON('https://www.googleapis.com/youtube/v3/channels?part=id&forUsername=' + youtubeUser + '&key=AIzaSyAhzndn-GWCUbm3NW0ecBPwORGMXLKhn6Y', function(response) {
							...
							if (youtubeIDGet.items.length !== 0) {
			...
			if (linkType === 'social network') {
				if (linkRel.url.resource.indexOf('facebook') >= 0 || linkRel.url.resource.indexOf('instagram') >= 0 || linkRel.url.resource.indexOf('twitter') >= 0) {
					socialMedia.push(musicBrainz.relations[getRel].url.resource);
				}
		...
		//<i className={"avail-on fa fa-" + availResourcesType[i]} aria-hidden="true"></i>

		function getSMtype(resource) {
			if (resource.indexOf('facebook') >= 0) {
				return 'facebook';
			} else if (resource.indexOf('instagram') >= 0) {
				return 'instagram';
			} else if (resource.indexOf('twitter') >= 0) {
				return 'twitter';
			}
		};

		const socialMed = socialMedia.map((socialMedia, i) =>
			<a href={socialMedia} rel="noopener noreferrer" target="_blank"><i className={"side-on fa fa-" + getSMtype(socialMedia)} aria-hidden="true"></i></a>
		);

		ReactDOM.render(
			<div className="social-meida-div">{socialMed}</div>,
			document.getElementById('social-media-sidebar')
		);
